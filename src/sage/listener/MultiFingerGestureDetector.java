...
public class MultiFingerGestureDetector {
	...
	private final int TIMEOUT_TAP = ViewConfiguration.getTapTimeout();
	private final int TIMEOUT_DTAP = ViewConfiguration.getDoubleTapTimeout();
	private final int TIMEOUT_LONG = ViewConfiguration.getLongPressTimeout();
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		case MotionEvent.ACTION_POINTER_UP:
			if (mIgnore || mLongActive || mPointerCnt == 0 || e.getPointerCount() != mPointerCnt)
				return false;
			...
			if (Math.abs(x - mStartX) <= 5 && Math.abs(y - mStartY) <= 5 && now - mStartTime <= TIMEOUT_TAP) {
				if (mListener != null)
					mListener.onMFingGesture(e.getPointerCount(), false);
				return true;
			}// id
			break;
		...
		default:
			...
			if (pCnt == mPointerCnt && now - mStartTime >= TIMEOUT_LONG) {
				if (mListener != null)
					mListener.onMFingGesture(pCnt, true);
				mLongActive = true;
				return true;
			}// if
			break;
