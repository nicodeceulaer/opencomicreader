...
public class ImgTransform {
	...
	public boolean isOnLeft() {
		return (srcRect.left == 0);
	}

	public boolean isOnRight() {
		return (srcRect.left >= mSrcWidth - srcRect.width());
	}

	public boolean isOnTop() {
		return (srcRect.top == 0);
	}

	public boolean isOnBottom() {
		return (srcRect.top >= mSrcHeight - srcRect.height());
	}

	public int getRightBoundary() {
		return (int) (mSrcWidth - srcRect.width());
	}
	...
	public boolean ismEnlargeSmallerThanScreen() {
		return mEnlargeSmallerThanScreen;
	}

	public void setmEnlargeSmallerThanScreen(boolean mEnlargeSmallerThanScreen) {
		this.mEnlargeSmallerThanScreen = mEnlargeSmallerThanScreen;
		recalculateScale();
	}
	...
	public boolean hasScaleChanged() {
		return mSrcScale != mOrgSrcScale;
	}

	public float getReverseScale() {
		return srcRect.width() / mViewWidth;
	}
	...
	private void calcScale() {
		...
		if (scaleMode == SCALE_AUTO)
			scaleMode = (mViewWidth > mViewHeight) ? SCALE_WIDTH : SCALE_HEIGHT; // Auto Width/Height

		switch (scaleMode) {
		// No Scale
		case SCALE_NONE:
			mOrgSrcScale = mSrcScale = 1;
			break;

		// Scale Width
		case SCALE_WIDTH:
			if (mEnlargeSmallerThanScreen || mViewWidth < mSrcWidth) {
				mOrgSrcScale = mSrcScale = mViewWidth / mSrcWidth;
			}
			break;

		// Scale Height
		case SCALE_HEIGHT:
			if (mEnlargeSmallerThanScreen || mViewHeight < mSrcHeight) {
				mOrgSrcScale = mSrcScale = mViewHeight / mSrcHeight;
			}
			break;
		}// switch
	...
	private boolean calcPan(float distanceX, float distanceY, boolean useScale, int[] outX, int[] outY) {
		...
		if (h < mSrcHeight) {
			...
			else if (newY > yBound && srcRect.top < yBound)
				y = yBound - srcRect.top;
		}// if
		...
		if (w < mSrcWidth) {
			...
			// If new position is between the boundary, let it pass.
			if (newX >= 0 && newX <= xBound)
				x = (int) distanceX;

			// if the new position is less then zero, but the current top is still greater then zero, calc the remaining distance to reach zero.
			else if (newX < 0 && srcRect.left > 0)
				x = srcRect.left * -1;

			// if the new position is over the Y limit but the current top is less then the limit, cal the remaining distance to reach the limit.
			else if (newX > xBound && srcRect.left < xBound)
				x = xBound - srcRect.left;
		}// if
	...
	public boolean appyScaleRatio(float ratio) {
		return applyScale(mSrcScale * ratio);
	}// func

	public boolean applyScale(float srcScale) {
		int srcRectWidth, srcRectHeight, viewRectWidth, viewRectHeight, srcRectLeft, srcRectTop, viewRectLeft, viewRectTop, srcScaleWidth = Math
				.round(mSrcWidth * srcScale), srcScaleHeight = Math.round(mSrcHeight * srcScale);

		// .................................................
		// Put a limit of how small to scale the image
		if (srcScaleWidth < 200 || srcScaleHeight < 200)
			return false;
		...
		if (srcScaleWidth < mViewWidth) {
			...
			if (mIsInit) {
				...
				if (srcRectLeft < 0)
					srcRectLeft = 0;
				else if (srcRectLeft > mSrcWidth - srcRectWidth)
					srcRectLeft = (int) mSrcWidth - srcRectWidth;
			}// if
		}// if
		...
		if (srcScaleHeight < mViewHeight) {
			...
			if (mIsInit)
				...
				if (srcRectTop < 0)
					srcRectTop = 0;
				else if (srcRectTop > mSrcHeight - srcRectHeight)
					srcRectTop = (int) mSrcHeight - srcRectHeight;
			}// if
		}// if
	...
	public boolean appyPanAnimate(final GestureImageView view, float distanceX, float distanceY, boolean useScale) {
		...
		if (calcPan(distanceX, distanceY, useScale, outX, outY)) {
			...
			final int startX = srcRect.left, startY = srcRect.top, difX = outX[0], difY = outY[0];
