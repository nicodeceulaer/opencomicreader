...
import com.runeai.runereader.BuildConfig;

/**
 * GameSurfaceRendererBitmap is a renderer that handles the rendering of a background bitmap to the screen (e.g., a game map). It is able to do this even if the
 * bitmap is too large to fit into memory. The game should subclass the renderer and extend the drawing methods to add other game elements.
 * 
 * @author micabyte
 */
public class BitmapSurfaceRenderer extends SurfaceRenderer {
	...
	/**
	 * What percent of total memory should we use for the cache? The bigger the cache, the longer it takes to read -- 1.2 secs for 25%, 600ms for 10%, 500ms for
	 * 5%. User experience seems to be best for smaller values.
	 */
	 ...
	 * 
	 * @param inputStream
	 *            InputStream to the raw data of the bitmap
	 * @throws IOException
	 */
	public void setBitmap(InputStream inputStream) throws IOException {
		...
		if (BuildConfig.DEBUG)
			Log.d(TAG, "Decode inputStream for Background Bitmap");
	 ...
	 * 
	 * @param rect
	 *            The portion of the background bitmap to be cached
	 ...
	 * 
	 * @param error
	 *            The OutOfMemoryError exception data
	 */
	...
	/**
	 * This method fills the passed-in bitmap with sample data. This function must return data fast; this is our fall back solution in all the cases where the
	 * user is moving too fast for us to load the actual bitmap data from memory. The quality of the user experience rests on the speed of this function.
	 */
	 ...
	 * <p/>
	 * Minimum size is equal to the viewport; otherwise it is dimensioned relative to the available memory. {@link CacheBitmap} is locked while the calculation
	 * is done, so this has to be fast.
	 * 
	 * @param rect
	...
	Rect calculateCacheDimensions(Rect rect) {
		...
		int vh = rect.height();
		if (BuildConfig.DEBUG)
			Log.d(TAG, "old cache.originRect = " + rect.toShortString());
		...
		if (vw + mw > sz.x)
			mw = Math.max(0, sz.x - vw);
		if (vh + mh > sz.y)
			mh = Math.max(0, sz.y - vh);
	...
	/**
	 * The cached bitmap object. This object is continually kept up to date by CacheThread. If the object is locked, the background is updated using the low
	 * resolution background image instead
	 */
	 ...
	 * <p/>
	 * The CacheThread starts an update when the {@link CacheBitmap#state_} is {@link CacheState#BEGIN_UPDATE} and updates the bitmap given the current window.
	 * <p/>
	 * The CacheThread needs to be careful how it locks {@link CacheBitmap} in order to ensure the smoothest possible performance (loading can take a while).
	 */
