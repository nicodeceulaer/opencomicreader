...
/**
 * MicaSurfaceView encapsulates all of the logic for handling 2D game maps. Pass it a SurfaceListener to receive touch events and a SurfaceRenderer to handle
 * the drawing.
 * 
 * @author micabyte
 */
public class MicaSurfaceView extends android.view.SurfaceView implements SurfaceHolder.Callback, OnGestureListener {
	...
	// Gesture Types
	public static final int TAPLEFT = 1, TAPRIGHT = 4, FLINGLEFT = 2, FLINGRIGHT = 8, TWOFINGTAP = 16, TWOFINGHOLD = 32, MASKLEFT = 3, MASKRIGHT = 12;
	...
	public void setMapPosition(Point p) {
		this.renderer_.setMapPosition(p.x, p.y);
	}
	...
	/*
	 * public void setEvent(Runnable r) { this.threadEvent_ = r; if (this.thread_ != null) this.thread_.setEvent(r); }
	 */

	// Clears the runnable event, if any, from the rendering thread.
	/*
	 * public void clearEvent() { this.thread_.clearEvent(); }
	 */
	...
	class GameSurfaceViewThread extends Thread {
		...
		/*
		 * public void onWindowFocusChanged(boolean hasFocus) { synchronized (this) { this.hasFocus_ = hasFocus; if (this.hasFocus_ == true) { notify(); } } }
		 * 
		 * public void surfaceDestroyed() { synchronized (this) { this.running_ = false; } }
		 * 
		 * // Queue an "event_" to be run on the rendering thread. public void setEvent(Runnable r) { synchronized (this) { this.event_ = r; } }
		 * 
		 * public void clearEvent() { synchronized (this) { this.event_ = null; } }
		 */
	...
	private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
		...
		public boolean onScale(ScaleGestureDetector detector) {
			...
			if (scaleFactor != 0f && scaleFactor != 1.0f) {
				...
				MicaSurfaceView.this.renderer_.zoom(scaleFactor, this.screenFocus);
				invalidate();
			}
	...
	class TouchHandler {
		...
		boolean fling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
			...
			synchronized (this) {
				...
				this.scroller_.fling(this.viewCenterAtFling_.x, this.viewCenterAtFling_.y, (int) -velocityX, (int) -velocityY, 0, this.backgroundSizeAtFling_.x
						- this.viewSizeAtFling_.x, 0, this.backgroundSizeAtFling_.y - this.viewSizeAtFling_.y);
				this.touchThread_.interrupt();
			}
			return true;
		}
