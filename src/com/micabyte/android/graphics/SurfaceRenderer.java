 ...
 * <p/>
 * - BitmapSurfaceRenderer can be extended for apps that require background images. - TileMapSurfaceRenderer can be extended for apps that need to display
 * TileMaps.
 * 
 * @author micabyte
 */
public abstract class SurfaceRenderer {
	...
	/*
	 * Set the position of the view * public void moveViewPosition(int fx, int fy, int dx, int dy) { this.viewPort_.moveViewPosition(fx, fy, dx, dy); }
	 * 
	 * public float getScaleFactor() { return this.viewPort_.getZoom(); }
	 * 
	 * public void setScaleFactor(float s) { /* float tempScaleFactor = s; // Check bounds Point viewSize = new Point(); Point backgroundSize =
	 * getBackgroundSize(); getViewSize(viewSize); float min = Math.max((float) viewSize.x / (float) backgroundSize.x, (float) viewSize.y / (float)
	 * backgroundSize.y); tempScaleFactor = Math.max(min, Math.min(tempScaleFactor, this.maxScaleFactor_)); this.scaleFactor_ = tempScaleFactor;
	 * 
	 * }
	 * 
	 * public void changeScaleFactor(float multiplier) { /* float tempScaleFactor = this.scaleFactor_ * multiplier; // Check bounds Point viewSize = new
	 * Point(); Point backgroundSize = getBackgroundSize(); getViewSize(viewSize); float min = Math.max((float) viewSize.x / (float) backgroundSize.x, (float)
	 * viewSize.y / (float) backgroundSize.y); tempScaleFactor = Math.max(min, Math.min(tempScaleFactor, this.maxScaleFactor_)); // Calculate new origin Point
	 * tempP = new Point(); this.getViewPosition(tempP); // Set scale factor this.scaleFactor_ = tempScaleFactor; this.setViewPosition(tempP.x, tempP.y);
	 * 
	 * }
	 */
